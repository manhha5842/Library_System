...
import { mockFineRecords } from '../../types/mockData';
import api from '../../config/apiConfig';
import FineRecordCard from './FineRecordCard';

interface FineRecordListProps {
    statuses: FineRecordStatus[];
    listHeaderComponent?: React.ComponentType<any> | React.ReactElement | null;
}

const FineRecordList: React.FC<FineRecordListProps> = ({ statuses, listHeaderComponent }) => {
    ...
    const fetchRecords = useCallback(async (isRefresh: boolean = false) => {
        ...
        try {
            ...
            console.error(`Error fetching fine records for statuses ${statuses.join(',')}:`, error);
            if (currentPage === 0) {
                const filteredMock = mockFineRecords.filter(r => statuses.includes(r.status));
    ...
    const renderEmptyComponent = () => (
        <Box flex={1} justifyContent="center" alignItems="center" mt={20}>
            <Text>Không có phiếu phạt nào.</Text>
        </Box>
    );
    ...
    return (
        <FlatList
            data={records}
            renderItem={({ item }) => <FineRecordCard record={item} />}
